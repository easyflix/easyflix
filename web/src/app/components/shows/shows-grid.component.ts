    ...
    <nav>
      <div class="item"
           *ngFor="let show of shows$ | async; trackBy: trackByFunc" tabindex="0"
           [routerLink]="['/', {outlets: {show: [show.id]}}]"
...
export class ShowsGridComponent implements OnInit {
...
/*  play(movie: Movie) {
  ...
  openShow(show: Show): void {
    this.router.navigate(['/', {outlets: {show: [show.id]}}], { queryParamsHandling: 'preserve' });
  }
